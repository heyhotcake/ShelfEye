...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========== SCANNER GRID ENDPOINTS ==========
  // Get all scanner grids
  app.get("/api/scanner-grids", async (_req, res) => {
    try {
      const grids = await storage.getScannerGrids();
      res.json(grids);
    } catch (error) {
      res.status(500).json({ message: "Failed to get scanner grids", error });
    }
  });

  // Get scanner grids for a specific camera
  app.get("/api/scanner-grids/camera/:cameraId", async (req, res) => {
    try {
      const grids = await storage.getScannerGridsByCamera(req.params.cameraId);
      res.json(grids);
    } catch (error) {
      res.status(500).json({ message: "Failed to get scanner grids", error });
    }
  ...
  // Get a specific scanner grid with its cells
  app.get("/api/scanner-grids/:id", async (req, res) => {
    try {
      const grid = await storage.getScannerGrid(req.params.id);
      if (!grid) {
        return res.status(404).json({ message: "Scanner grid not found" });
      }
      const cells = await storage.getScannerCells(grid.id);
      res.json({ ...grid, cells });
    } catch (error) {
      res.status(500).json({ message: "Failed to get scanner grid", error });
    }
  });

  // Create a new scanner grid with auto-generated cells
  app.post("/api/scanner-grids", async (req, res) => {
    try {
      const { startXCm, startYCm, ...gridData } = req.body;
      
      // Validate required fields
      if (!gridData.cameraId || !gridData.name || !gridData.gridType || !gridData.cellWidthCm || !gridData.cellHeightCm) {
        return res.status(400).json({ 
          message: "Missing required fields: cameraId, name, gridType, cellWidthCm, cellHeightCm" 
        });
      }
      
      if (!['scanner', 'worker_tag'].includes(gridData.gridType)) {
        return res.status(400).json({ message: "gridType must be 'scanner' or 'worker_tag'" });
      }
      
      const grid = await storage.createScannerGridWithCells(
        gridData,
        startXCm ?? 5, // Default start position
        startYCm ?? 5
      );
      
      const cells = await storage.getScannerCells(grid.id);
      res.status(201).json({ ...grid, cells });
    } catch (error) {
      res.status(500).json({ message: "Failed to create scanner grid", error });
    }
  });

  // Update a scanner grid
  app.patch("/api/scanner-grids/:id", async (req, res) => {
    try {
      const grid = await storage.updateScannerGrid(req.params.id, req.body);
      if (!grid) {
        return res.status(404).json({ message: "Scanner grid not found" });
      }
      res.json(grid);
    } catch (error) {
      res.status(500).json({ message: "Failed to update scanner grid", error });
    }
  });

  // Delete a scanner grid (cascade deletes cells)
  app.delete("/api/scanner-grids/:id", async (req, res) => {
    try {
      const deleted = await storage.deleteScannerGrid(req.params.id);
      if (!deleted) {
        return res.status(404).json({ message: "Scanner grid not found" });
      }
      res.json({ ok: true });
    } catch (error) {
      res.status(500).json({ message: "Failed to delete scanner grid", error });
    }
  });

  // Link scanner grid to worker tag grid
  app.post("/api/scanner-grids/:id/link", async (req, res) => {
    try {
      const { linkedGridId } = req.body;
      
      if (!linkedGridId) {
        return res.status(400).json({ message: "linkedGridId is required" });
      }
      
      // Verify both grids exist and are of correct types
      const grid = await storage.getScannerGrid(req.params.id);
      const linkedGrid = await storage.getScannerGrid(linkedGridId);
      
      if (!grid || !linkedGrid) {
        return res.status(404).json({ message: "One or both grids not found" });
      }
      
      if (grid.gridType === linkedGrid.gridType) {
        return res.status(400).json({ message: "Cannot link grids of the same type" });
      }
      
      // Link both grids to each other
      await storage.updateScannerGrid(grid.id, { linkedGridId });
      await storage.updateScannerGrid(linkedGrid.id, { linkedGridId: grid.id });
      
      // Also link corresponding cells
      const gridCells = await storage.getScannerCells(grid.id);
      const linkedGridCells = await storage.getScannerCells(linkedGrid.id);
      
      for (const cell of gridCells) {
        const linkedCell = linkedGridCells.find(c => c.cellNumber === cell.cellNumber);
        if (linkedCell) {
          await storage.updateScannerCell(cell.id, { linkedCellId: linkedCell.id });
          await storage.updateScannerCell(linkedCell.id, { linkedCellId: cell.id });
        }
      }
      
      res.json({ ok: true, message: "Grids linked successfully" });
    } catch (error) {
      res.status(500).json({ message: "Failed to link grids", error });
    }
  });

  // ========== SCANNER CELL ENDPOINTS ==========
  // Get cells for a scanner grid
  app.get("/api/scanner-cells/grid/:gridId", async (req, res) => {
    try {
      const cells = await storage.getScannerCells(req.params.gridId);
      res.json(cells);
    } catch (error) {
      res.status(500).json({ message: "Failed to get scanner cells", error });
    }
  });

  // Update a scanner cell (for dragging on canvas)
  app.patch("/api/scanner-cells/:id", async (req, res) => {
    try {
      const cell = await storage.updateScannerCell(req.params.id, req.body);
      if (!cell) {
        return res.status(404).json({ message: "Scanner cell not found" });
      }
      res.json(cell);
    } catch (error) {
      res.status(500).json({ message: "Failed to update scanner cell", error });
    }
  });

  // ========== SCANNER DETECTION LOG ENDPOINTS ==========
  // Get recent scanner detection logs
  app.get("/api/scanner-detection-logs", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 100;
      const logs = await storage.getScannerDetectionLogs(limit);
      res.json(logs);
    } catch (error) {
      res.status(500).json({ message: "Failed to get scanner detection logs", error });
    }
  });

  // Get detection logs for a specific cell
  app.get("/api/scanner-detection-logs/cell/:cellId", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 100;
      const logs = await storage.getScannerDetectionLogsByCell(req.params.cellId, limit);
      res.json(logs);
    } catch (error) {
      res.status(500).json({ message: "Failed to get scanner detection logs", error });
    }
  ...
  // Process scanner grid detection (yellow color detection)
  // Security: Only allows internal rectified images, no user-supplied paths
  app.post("/api/scanner-grids/process/:cameraId", async (req, res) => {
    try {
      const { cameraId } = req.params;
      
      // Security: Validate cameraId is a valid UUID format
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      if (!uuidRegex.test(cameraId)) {
        return res.status(400).json({ message: "Invalid camera ID format" });
      }
      ...
      const grids = await storage.getScannerGridsByCamera(cameraId);
      if (grids.length === 0) {
        return res.json({ ok: true, message: "No scanner grids configured for this camera", results: [] });
      }
      
      // Build cell configurations for Python script
      const scannerCells: any[] = [];
      const workerTagCells: any[] = [];
      ...
      for (const grid of grids) {
        ...
        for (const cell of cells) {
          const cellConfig = {
            ...
            x_cm: cell.xCm,
            y_cm: cell.yCm,
            width_cm: cell.widthCm,
            height_cm: cell.heightCm,
            linked_cell_id: cell.linkedCellId,
          };
          
          if (grid.gridType === 'scanner') {
            scannerCells.push(cellConfig);
          } else {
            workerTagCells.push(cellConfig);
          }
        }
      }
      
      // Security: Only use known internal rectified image path, never user-supplied
      const rectifiedImagePath = path.join(process.cwd(), 'data', `validation_rectified_debug_${cameraId}.png`);
      
      // Check if image exists
      try {
        await fs.access(rectifiedImagePath);
      } catch {
        return res.status(400).json({ message: "No rectified image available. Run calibration first." });
      }
      
      // Get pixels per cm from camera homography or use default
      // 6-page layout: 89.1cm x 42cm at typical 2832x1416 rectified = ~31.8 px/cm
      const pxPerCm = 31.8; // TODO: Calculate from camera homography when available
      
      // Call Python scanner detection script
      const cellsConfig = JSON.stringify({
        scanner_cells: scannerCells,
        worker_tag_cells: workerTagCells,
      });
      
      const pythonScript = path.join(process.cwd(), 'python', 'scanner_color_detection.py');
      
      const pythonProcess = spawn('python3', [
        pythonScript,
        '--image', rectifiedImagePath,
        '--cells', cellsConfig,
        '--px-per-cm', pxPerCm.toString(),
      ]);
      ...
      pythonProcess.stderr.on('data', (data) => {
        stderr += data.toString();
        console.log(`[ScannerDetection] ${data.toString()}`);
      });
      ...
      pythonProcess.on('close', async (code) => {
        if (code === 0) {
          try {
            ...
            for (const scannerResult of results.scanner_cells) {
              await storage.createScannerDetectionLog({
                cellId: scannerResult.cell_id,
                status: scannerResult.status,
                yellowDetected: scannerResult.detected,
                yellowCoverage: scannerResult.coverage,
                workerId: scannerResult.checked_out_by ? String(scannerResult.checked_out_by) : null,
                workerArucoId: scannerResult.checked_out_by || null,
              });
            }
            ...
            res.status(500).json({ message: "Failed to parse detection results", error: String(parseError) });
          }
        } else {
          res.status(500).json({ message: "Scanner detection failed", error: stderr });
        }
      });
      
    } catch (error) {
      res.status(500).json({ message: "Scanner detection error", error: String(error) });
    }
  });
