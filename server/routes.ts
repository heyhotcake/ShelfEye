...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/calibrate/:cameraId", async (req, res) => {
    ...
    try {
      ...
      console.log(`[Calibration] Paper: ${paperDims.widthCm}x${paperDims.heightCm} cm`);
  ...
  app.post("/api/calibrate/:cameraId/verify-positions", async (req, res) => {
    ...
    try {
      ...
      const previewWidth = Math.round(paperDims.widthCm * previewScale);
      const previewHeight = Math.round(paperDims.heightCm * previewScale);
      console.log(`[VerifyPositions] Paper: ${paperDims.widthCm}x${paperDims.heightCm} cm`);
      console.log(`[VerifyPositions] Preview output size: ${previewWidth}x${previewHeight} px (30 px/cm)`);
  ...
  app.get("/api/rectified-preview/:cameraId", async (req, res) => {
    ...
    try {
      ...
      console.log(`[Rectified Preview] Preview output size: ${previewWidth}x${previewHeight} px (30 px/cm)`);
  ...
  app.post("/api/tool-categories/scanner-grid", async (req, res) => {
    try {
      ...
      const workerTagHeaderHeight = 2; // 2cm header, no margins
