...
**Notes**: Visual alerts use the LED strip (red flash mode). The DHT20 uses I2C bus 1 (GPIO 2/3) for temperature/humidity readings - this is the standard I2C bus with built-in pull-up resistors, no additional configuration required. The Ario 2401 buzzer is GPIO-controlled via `python/buzzer_control.py` and triggers automatically with red flash alerts.
