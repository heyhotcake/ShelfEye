...
- **6-Page (3×2 A4) Multi-Sheet Template System (Oct 2025)**: Implemented large-format template design using 6 A4 portrait sheets arranged in 3×2 grid (645mm × 601.5mm total area). Features include: 7.5mm gutters between sheets for print-and-tape assembly (no cutting required), ArUco markers (IDs 17-20) positioned on corner sheets only (1,3,4,6) with 5mm inset, 1cm safe zones per sheet preventing placement near edges, sheet-constrained rectangle placement (no cross-gutter placement allowed), PDF export with 6 separate pages including sheet numbers and assembly instructions. Coordinate system accounts for gutters in global positioning. Architect-reviewed and approved for production use.
