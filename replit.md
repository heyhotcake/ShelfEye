...
A Raspberry Pi-based automated tool monitoring system utilizing computer vision, QR codes, and ArUco markers for real-time tool tracking across multiple cameras. It features simple QR code validation (no cryptographic signatures), temporal smoothing for presence detection, and multi-channel alerting (email, Google Sheets, sound). The system includes a React web dashboard for calibration, configurable slot management, analytics, and system administration. Its core purpose is to prevent tool loss and improve accountability in workshops by tracking tool presence and checkout status.
...
- **QR Detection Enhancement & Error Handling Fix (Oct 2025)**: Comprehensive improvements to QR code detection reliability and user-facing error messages. Enhanced Python validation script with: 80 px/cm resolution (2x increase), INTER_NEAREST interpolation on all warp paths, multi-frame capture (5 frames) with Laplacian sharpness selection, camera warm-up (discard first 2 frames), CAP_PROP_BUFFERSIZE=1 for fresh frames, dual decoders (pyzbar + OpenCV detectAndDecodeMulti), and 6 preprocessing methods (original, grayscale, adaptive, inverted, Otsu-inverted, CLAHE). Fixed OpenCV fallback to use detectAndDecodeMulti for multiple QR detection with proper bbox handling via reshape(-1, 2). Fixed frontend validation error handling to check for `data.error` field before accessing count fields, preventing "undefined/undefined" error messages when camera fails to open. System now shows clear error messages for all failure modes. Expected improvement from 2/7 to 7/7 QR detections. Production-ready with architect verification.

- **Active Camera Card & Template-Camera Independence (Oct 2025)**: Implemented Active Camera card on dashboard showing current calibrated camera and template combination. Card displays camera name, paper size format, calibration timestamp, and "Calibrated" badge with highlighted border. Only appears after full 3-stage calibration completion (ArUco, QR visible, QR covered). Moved `last_calibration_*` config saving from ArUco endpoint to `validate-qrs-covered` endpoint to ensure dashboard card only appears when all validation stages complete successfully. Templates are now fully camera-independent: removed camera selection from template designer, templates can be used with any camera during calibration, filtering by paper size only. Frontend shows all saved templates regardless of selected camera. Calibration page allows independent camera and template selection for maximum flexibility. Backend validates paper size compatibility between templates and calibration. Query invalidation immediately refreshes dashboard after successful calibration. Production-ready with proper 3-stage verification.

- **HMAC Removal for Simplified QR Codes (Oct 2025)**: Removed all HMAC signature validation code to optimize QR code scanning reliability. QR codes now contain only simple ID strings (e.g., "pen-001", "worker-john") without cryptographic signatures, reducing data density by 5.5x for better scanning at 30mm size. Removed `validate_hmac_signature()` function from Python validator, removed HMAC secret parameter from backend validation endpoints, and removed `includeHmac` flag from frontend QR generation. Physical security of printed QR codes provides sufficient protection without cryptographic overhead. System maintains robust validation through database lookups for worker QR codes. Production-ready with cleaner, more maintainable codebase.

- **Camera USB MJPG Format Fix & LED Control (Oct 2025)**: Fixed critical USB camera compatibility issues and LED light control during calibration. Updated all Python camera scripts (camera_preview.py, rectified_preview.py, aruco_calibrator.py, validate_slot_qrs.py) to use MJPG format for 1920x1080 resolution support (camera supports MJPG@30fps, YUYV only @5fps). Fixed startup calibration to turn on white LED light strip before calibration and turn it off after completion, ensuring ArUco markers are visible. Added deleteDetectionLogsBySlotId() method to storage to prevent foreign key constraint errors when recalibrating (deletes detection logs before deleting slots). Changed all server spawn calls from 'python' to 'python3' for Raspberry Pi compatibility. Production-ready with comprehensive error handling.
...
- **Two-Step Calibration Validation (Oct 2025)**: Implemented comprehensive QR code validation during calibration to ensure proper setup. The calibration process now has three steps: (1) ArUco corner marker detection and homography calculation, (2) Validation that all slot QR codes are readable when slots are empty, (3) Validation that QR codes are properly covered when tools are placed. Python validation script (`validate_slot_qrs.py`) decodes QR codes from rectified camera view and matches against expected slot identifiers. QR codes contain simple ID strings (e.g., "pen-001") without cryptographic signatures for optimal scanning reliability at 30mm size. Backend provides two validation endpoints (`/validate-qrs-visible` and `/validate-qrs-covered`) that pass `slot.expectedQrId` to match QR payload. Frontend guides users through step-by-step process with clear instructions, real-time validation feedback, and detailed error messages showing which slots are missing or visible. Includes reset functionality to restart calibration process. Production-ready with comprehensive error handling.
...
- **QR Type System**: "slot" and "worker" types with simple ID strings (no cryptographic signatures).
...
- **QR Code Format**: Simple ID payloads (e.g., "pen-001", "worker-john") optimized for 30mm scanning reliability.
